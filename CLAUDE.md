...
- **`title_provenance`** دخل عقدَ الالتقاط ⟵ حلقةُ التسميم الذاتيّ للعنوان أُغلقت
  (الحصادُ فاشلٌ‑مغلقاً: كتبُ التطبيق تُستبعَد إلّا بشهادة `typed`، `02d8081`).
- التفصيلُ وحدودُ ما لا يُدَّعى في `docs/EVAL_REGISTRY.md` (قسم 2026-09-01).
- **مراجعةُ المساء (`02d8081`)**: الصارمُ كان يُغذّى بنصٍّ مفروزٍ لا يُنتجه القياس
  (8/34 بدل 32/34) — أُصلح بـ`pdf_first_page_text`؛ **e2e-E مجموعةُ تطويرٍ** (ضُبطت
  بعد النظرة) والمختومُ الوحيد للصارم e2e-D؛ e2e-F مُسجَّلةٌ بمسار `process_image`.
  الالتقاطُ يقرأ `title_suggestion`، و`harvest_dates` يستبعد `autofilled` فعلاً.
...
### آخر تعديلات (2026-09-01، ليلاً متأخّراً) — ثلاثُ نظراتٍ مختومة
- **الصارمُ مختومٌ بعين Claude (e2e‑F)**: 28/58 إصابة · خاطئ 2 · دقّةُ المُطلَق **93%**.
  الحقيقةُ قُرئت من صورة الرأس قبل النظر إلى المخرَج (`truth_src='eye:claude-vision'`).
  و**`printed_anchor` كان 0/10** (لا تقنين + بتر) ⟵ `sanitize_printed_anchor` (تقنينُ
  البادئات المعتمدة + حارس 3–8 خانات). **وسمُ القاعدة ≠ المطبوع في 33/58.**
- **بوّابةُ التاريخ سقطت**: على 100 وارد لم يُرَ — اقتراحٌ 34% · `parse ok` 14% · أخضر **7%**
  بدقّة **4/7**. أرقامُ الحجز (64%/91.7%) كانت مشروطةً بقصاصاتٍ محصودة. القمع: صمتُ
  الكاشف ~37% ⟵ امتناعُ المحلِّل 59%. التاريخُ أضعفُ الحقول تقييماً.
- **توحيدُ شكل نصّ الذاكرة** (Tesseract خامّ عند التخزين ≠ `clean_text` عند الاستعلام)
  شُحن في `_normalize_ar`: E‑100 المُصدِرة 44 ⟵ 50 · المستلمة 60.6 ⟵ 64.9 (تطوير حتّى E‑100c).

### آخر تعديلات (2026-09-01، ليلاً) — الجهتان
- **مختومٌ (E‑100b، 2026-09-01)**: المُصدِرة **48.0%** · المستلمة **62.9%** top‑1 بمسار
  الإنتاج (OCR حيّ) — المستلمةُ سقطت عن بوّابة 63 بكتابٍ واحد؛ لا تُقتبَس أرقامُ Tier A
  (73.6%) رقماً رسميّاً. **الأنبوبُ لم يكن يعرف نوعَ الكتاب** وقت الاستخراج (E‑100:
  المستلمة 29.8% بدونه، 60.6% به) — صار يُرسَل من الواجهة مع كلّ رفع (`book_kind`).
- **أوّلُ قياسٍ للجهة المستلمة** بمسار الإنتاج (`resolve_entity_candidates`، صار
  دالّةً قابلةً للقياس): المستلمة **top‑1 65.2%** (n=932) · المُصدِرة **51.0%**
  (الذاكرةُ وحدَها 49.5% = لا انجراف عن 49.0%). Tier A تطويرٌ؛ الأداة
  `scripts/eval/entity_eval.py`. الصادرُ الداخليُّ أضعفُ الجميع (29.6%/40.9%).
- **حلقةُ تعلّم الجهات أُصلحت**: الذاكرةُ كانت تُهمل التصحيحَ بعد أوّل حفظ (عودةٌ
  مبكرة، وطريقُ التعديل لا يستدعي الالتقاط) ولا تسأل عن المنشأ. الآن
  `refresh_letterhead_memory` في التعديل، ووسمُ `issuing/receiving_entity_provenance`
  من الواجهة، وجانبٌ `autofilled` لا يُكتب.
- **التعلّمُ من التصحيحات عموماً**: الذاكرةُ وحدها تعلّمٌ حيّ؛ العدد/التاريخ/الموضوع
  **تسجيلٌ** (9 صفوف تغذيةٍ راجعة كلّها موضوع، 25 التقاطاً) لا يصير تعلّماً إلّا
  بحصادٍ وإعادةِ تدريبٍ يدويّين.
...
## Current State
### الخطّةُ المرجعيّة (2026-08-31) — v2
- **المرجعُ الحاليّ:** `COMPANY_ROLLOUT_PLAN_v2_2026-08-31.md` (الأدوار · الأقسام · الجهات · الصفحات · الحواريّات · الانتقالات · الدفعات القادمة). القديمةُ `…_2026-08-24.md` تاريخٌ لا مرجع.
- **قواعدُ الحرفة (§9.4 هناك) تسري على كلّ ما يُكتب:** مصدرٌ وحيدٌ لكلّ قاعدةٍ في `core/scoping.py` ولا شرطَ دورٍ في قالب · «منجَز» = طرفٌ إلى طرفٍ في المتصفّح بلقطةٍ تُقرأ · كلُّ حارسٍ يُطفَّر · لا `except` واسعٌ حول أثر · الإلحاقُ لا الكتابةُ فوق الوثائق · المخرَجُ الطويل إلى ملفّ (الجهاز 8GB).
- **التالي:** الدفعة 4‑ج (مسؤولُ الأرشفة — أولويّةُ المالك) ثمّ 4‑ب (بريدُ الوحدات + طاولتا الرئيسين) ثمّ الدمجُ الثاني بقرار المالك.
