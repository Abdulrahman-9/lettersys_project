...
class EntityMatcher:
    ...
    def match_from_memory(self, text: str, entity_type: str = 'issuer',
        ...
        لنفس الجهة، فالتشابه يكشفها.

        ⚠️ **الرقم القديم «85%/77%» مسحوب** (2026-08-17): كان تسريبَ مطابقةٍ ذاتيّة —
        الفهرس يحوي صفّ المستند المُقاس نفسه فيُطابقه بتشابه 1.0. الصادق بعد
        `exclude_book_id`: **top-1 60% · top-3 73%** على 30 نصّاً حقيقيّاً، و**49.0%**
        على 1000 استعلامٍ مُجمَّد. تبقى الميزة كبيرةً (نحو 3× خطّ مطابقة الاسم) لكنّ
        حجمها كان مُضخَّماً. يكبر مع كلّ كتاب يُحفَظ (حلقة الالتقاط) — بلا حدّ ولا إعادة تدريب.
