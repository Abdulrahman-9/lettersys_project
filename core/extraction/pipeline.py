...
class AIExtractionService:
    ...
    def _read_handwritten_sender_number(self, image_path, entity_id, want_date_crop=False):
        ...
        try:
            ...
            # ── قراءة قصاصة الكاشف: **مقيسةٌ ومُغلَقة الإصدار** (2026-08-18) ──
            # جُرِّبت على e2e‑A بنظرتين مُعلَنتين، وكلتاهما خرقت بوّابة
            # «واثقٌ‑ومخطئ لا يرتفع» (الأساس 3):
            #   قراءةٌ مفردة (حشو 0.30/0.40)  : إصابة 4⟵28 لكن واثق‑مخطئ 3⟵13
            #   تصويتُ قصاصتين (حشوان)        : إصابة 18 وواثق‑مخطئ 8 — البترُ
            #     يثبت عبر الحشوين («1» من «1220» اتّفقت فيهما) لأن انحياز القراءة
            #     القصيرة حتميٌّ في المنطقة لا في حدود القصّ.
            # قراءةٌ خاطئة بثقة 0.99 تسمّم القاعدة وتقتل ثقة الكاتب — فالحقل يبقى
            # صامتاً، والصندوق يُلتقط لدولاب التدريب (T2.4: إعادة تدريب CRNN على
            # قصاصات الكاشف؛ بوّابة 300 قصاصة مؤكَّدة، يُقاس على المجموعة C).
            # لا تُعِد فتح هذا المسار بلا رافعةٍ جديدةٍ مقيسةٍ على B.
