...
⚠️ **تاريخيّ — لنقل SQLite ⟵ PostgreSQL قبل 2026-07 فقط.**

**أيُّ نقلٍ بين قاعدتَي PostgreSQL يكون بـ``pg_dump -Fc``/``pg_restore`` حصراً.**
الخطواتُ 1 و4 و«استيراد البيانات» أدناه تدعو إلى ``dumpdata``/``loaddata``،
وهما **لا يصلحان لبيانات الإنتاج**: ``loaddata`` يتخطّى ``save()`` (``raw=True``)
فيكتب الأسرارَ **صريحةً** في القاعدة بلا إعادةِ تشفير، و``dumpdata`` يُسقط
المحذوفَ ناعماً، والتحميلُ يدمج المستخدمين **بالاسم** فيكتب فوق حساب الخادم.
التفصيلُ والقياسُ في ``Merge9.md`` §8 (و§0 يجعله قيداً صارماً).
